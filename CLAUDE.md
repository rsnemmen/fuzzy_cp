...
1. **Argument parsing** (`get_args`) — argparse, positional `names` file + flags `-s`, `-c`, `-m`, `-o`
...
6. **Output** — prints name | best-match | score (colorized), or writes filenames to file/stdout if `-o` given
